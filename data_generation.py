...
import shutil
import ray
from multiply_test import T_test1
...
def generate_dataset_vol_match_6DoF(output_dir: Path, cfg: DictConfig):
    ...
    while not success:
        # reset env
        env = TeleRobotEnv(cfg, dataset_split=dataset_split, gui=True)
        ...
        _, obj_details, kit_body_id = env.reset_scene_6DoF(output_dir, unit_kit=True, load_single_object=True)
        ...
        T_obj_to_concav = p.multiplyTransforms(*T_obj_to_world_ori, *obj_kit_frame) # obj rotated at origin, and kit at origin
